...
conferenceappModel.bookpage = (bookingdata, result)=>{
    var sqlQuery = `SELECT * FROM hrm_users where emp_code = '${bookingdata.emp_code}' and emp_status = 'active'`;
    ...
    db.query(sqlQuery, (error, response)=>{
        if(error){
            ...
            console.log(decrypted_login_password);
            if (response.length && decrypted_sql_password !== decrypted_login_password) {
                var userres = {
                    sts:"0",
                    message: "incorrect password",
                }
                result(null, userres);
              } else if (response.length && decrypted_sql_password == decrypted_login_password) {
            var userres = {
                sts:"1",
...
conferenceappModel.beejaLogin = (bookingdata, result)=>{
  ...
  beejadb.query(sqlQuery, (error, response)=>{
      ...
      if(error){
          ...
          var content = response[0]['decrypt_pass'];
          // var decrypted_sql_password = decrypt(content);
          // var encrypted_login_password = encrypt(bookingdata.emp_password);
          // var decrypted_login_password = decrypt(encrypted_login_password);
          if (response.length && content !== bookingdata.emp_password) {
              var userres = {
                  sts:"0",
                  message: "incorrect password",
              }
              ...
              content == bookingdata.emp_password) {
          var userres = {
              sts:"1",
              message: "Login Success",
              data:response
